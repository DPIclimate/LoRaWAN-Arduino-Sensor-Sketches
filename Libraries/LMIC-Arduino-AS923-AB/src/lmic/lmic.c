...
static CONST_TABLE(u4_t, iniChannelFreq)[4] = {
    // Join frequencies and duty cycle limit (0.1%)
    AS923_F1|BAND_MILLI, AS923_F2|BAND_MILLI,
    // Default operational frequencies
    AS923_F1|BAND_CENTI, AS923_F2|BAND_CENTI,
};
...
static void initDefaultChannels (bit_t join) {
    ...
    LMIC.channelMap = 0x03;
    u1_t su = join ? 0 : 2;
    for( u1_t fu=0; fu<2; fu++,su++ ) {
        LMIC.channelFreq[fu]  = TABLE_GET_U4(iniChannelFreq, su);
        LMIC.channelDrMap[fu] = DR_RANGE_MAP(DR_SF12,DR_SF7);
    }
...
static void initJoinLoop (void) {
//    LMIC.txChnl = os_getRndU1() % 3;
    LMIC.txChnl = os_getRndU1() % 2;
//    LMIC.txChnl = 0;
    LMIC.adrTxPow = 14;
//    setDrJoin(DRCHG_SET, DR_SF12);  //original SF is 7
    setDrJoin(DRCHG_SET, DR_SF10);  //original SF is 7
    initDefaultChannels(1);
    ASSERT((LMIC.opmode & OP_NEXTCHNL)==0);
    LMIC.txend = LMIC.bands[BAND_MILLI].avail + rndDelay(8);
}
